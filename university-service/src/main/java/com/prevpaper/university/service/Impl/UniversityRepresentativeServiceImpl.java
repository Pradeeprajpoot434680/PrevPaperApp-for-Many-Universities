...
public class UniversityRepresentativeServiceImpl implements UniversityRepresentativeService {
    ...
    @CacheEvict(value = "departments", key = "#universityId")
    public DepartmentTinyDTO createDepartment(UUID universityId, DepartmentRequest request) {
        log.info("Creating department for universityId={}", universityId);

        // 1. Verify and retrieve managed University instance
        University university = universityRepository.findById(universityId)
        ...
        // 2. Validate scope uniqueness specifically for THIS university
        if (departmentRepository.existsByCodeAndUniversityId(code, universityId)) {
            throw new ResourceAlreadyExist("Department with code '" + code + "' already exists in this university");
        }
        if (departmentRepository.existsByNameIgnoreCaseAndUniversityId(name, universityId)) {
            throw new ResourceAlreadyExist("Department with name '" + name + "' already exists in this university");
        }

        // 3. Build entity attached to managed University
        Department department = Department.builder()
                .name(name)
                .code(code)
                .university(university)
    ...
    public void assignDepartmentRep(UUID universityId, AssignRepRequest request, UUID adminId) {
        log.info("Assigning department representative: userId={}, scopeId={}, universityId={}",
                request.getUserId(), request.getScopeId(), universityId);

        boolean alreadyAssigned = representativeRepository
                .existsByUserIdAndScopeIdAndIsActiveTrue(request.getUserId(), request.getScopeId());

        if (alreadyAssigned) {
            throw new ResourceAlreadyExist("User is already an active representative for this department.");
        }
